...
export default class CancellationToken extends Emittable {
  ...
  protected emitEvent<K extends keyof CancellationTokenEventMap>(
    eventName: K,
    event: CancellationTokenEventMap[K]
  ) {
    super.emitEvent(eventName, event);
  }

  addEventListener<K extends keyof CancellationTokenEventMap>(
    eventName: K,
    listener: (event: CancellationTokenEventMap[K]) => any
  ) {
    super.addEventListener(eventName, listener);
  }

  removeEventListener<K extends keyof CancellationTokenEventMap>(
    eventName: K,
    listener: (event: CancellationTokenEventMap[K]) => any
  ) {
    super.addEventListener(eventName, listener);
